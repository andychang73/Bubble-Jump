...
import pkg20200326.gameproject.gameobject.bubbles.RedBubble;
 ...
 */
public class CreateRedBubble implements BubbleCreator{
    ...
    public Bubble bubbleCreator(Actor actor) 
    {
        ...
        {
            return new RedBubble(actor.getX() - Bubble.WIDTH - 10, actor.getY(), 
                                 actor.getLeftFrame(), actor.getRightFrame(),
                                (Global.random(0.5f, 0) * -1), (Global.random(0.5f, 0) * -1), RedBubble.INDEX);
        }
        if(actor.getState().getType().equals(MoveRight.TYPE))
        {
            return new RedBubble(actor.getX() + Bubble.WIDTH + 10, actor.getY(), 
                                 actor.getLeftFrame(), actor.getRightFrame(),
                                (Global.random(0.5f, 0)), (Global.random(0.5f, 0) * -1), RedBubble.INDEX);
        }
        return null;
    }
