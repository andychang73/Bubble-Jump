...
import pkg20200326.gameproject.gameobject.bubbles.PinkBubble;
 ...
 */
public class CreatePinkBubble implements BubbleCreator{
    ...
    public Bubble bubbleCreator(Actor actor) 
    {
        ...
        {
            return new PinkBubble(actor.getX() - Bubble.WIDTH - 10, actor.getY(), 
                                   actor.getLeftFrame(), actor.getRightFrame(),
                                  (Global.random(0.5f, 0) * -1), (Global.random(0.5f, 0) * -1), PinkBubble.INDEX);
        }
        if(actor.getState().getType().equals(MoveRight.TYPE))
        {
            return new PinkBubble(actor.getX() + Bubble.WIDTH + 10, actor.getY(), 
                                   actor.getLeftFrame(), actor.getRightFrame(),
                                  (Global.random(0.5f, 0)), (Global.random(0.5f, 0) * -1), PinkBubble.INDEX);
        }
        return null;
    }
