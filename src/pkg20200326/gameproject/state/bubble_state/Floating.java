 ...
 */
public class Floating extends BubbleState {
    ...
    public Floating(Bubble bubble, int bubbleIndex)
    {
        super(bubble);
        img = ImageResourceController.getInstance().tryGetImage(ImagePath.BUBBLE);
    ...
    public void paint(Graphics g, float cameraTopY, float cameraBottomY)
    {
        g.drawImage(img, (int) (bubble.getX() - bubble.getWidth()/2),
                         ...
                         (bubble_col_unit + bubble_row_unit * (bubbleIndex / bubble_col_num)), null);
    }
